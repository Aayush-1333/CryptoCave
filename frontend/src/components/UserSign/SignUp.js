        ...
        <div className='container-fluid flex justify-center p-16 bg-white dark:bg-black'>
            <form className='flex flex-col bg-indigo-400 dark:bg-yellow-600 gap-8 px-14 py-6 md:px-64 md:py-20 rounded-2xl' onSubmit={SubmitForm}>
                ...
                <div className='self-center flex flex-col gap-4'>
                    ...
                    <input type="text" className='rounded-lg md:p-2' name="username" placeholder='Your username' value={signData.username} onChange={OnChange} required />
                </div>

                <div className='self-center flex flex-col gap-4'>
                    <label htmlFor="email" className='md:text-xl'>Email</label>
                    <input type="email" className='rounded-lg md:p-2' name="email" placeholder='Your email' value={signData.email} onChange={OnChange} required />
                </div>

                <div className='self-center flex flex-col gap-4'>
                    <label htmlFor="phone_no" className='md:text-xl'>Contact No.</label>
                    <input type="tel" className='rounded-lg md:p-2' name="phone_no" placeholder='Your phone no.' value={signData.phone_no} onChange={OnChange} required />
                </div>

                <div className='self-center flex flex-col gap-4'>
                    <label htmlFor="password" className='md:text-xl'>Password</label>
                    <input type={passwdStyle} className='rounded-lg md:p-2' name="password" placeholder='Your password' value={signData.password} onChange={OnChange} required />
                </div>
